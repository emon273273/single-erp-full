...
export const loadAllPurchaseReport = createAsyncThunk(
  "purchase/loadAllPurchaseReport",
  async (arg) => {
    try {
      const query = queryGenerator(arg);
      const { data } = await axios.get(`purchase-invoice?${query}`);
...
const purchaseSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(loadAllPurchaseReport.pending, (state) => {
      state.loading = true;
    });

    builder.addCase(loadAllPurchaseReport.fulfilled, (state, action) => {
      state.loading = false;
      state.list = action.payload.data?.getAllPurchaseInvoice;
      state.information = action.payload.data?.aggregations?._sum;
    });

    builder.addCase(loadAllPurchaseReport.rejected, (state, action) => {
      state.loading = false;
      state.error = action.payload.message;
    });
  },
